...
class AUO_Dataloader:
    ...
    def load_AUO_data(
        ...
        date = datetime.strftime(self.system_start_time_dt, '%Y-%m-%d')
        export_dir = os.path.join('wip_data', date)
    ...
    def _get_tact_times(self, route_id, op_id, product_code, on_odf=True):
        tact_times = {}
        if on_odf:
            ...
            for eqp_id in self.eqp_group_table[eqp_group_id]:
                if (product_code, eqp_id) in self.tact_time_table and not math.isnan(self.tact_time_table[(product_code, eqp_id)]):
                    tact_times[eqp_id] = self.tact_time_table[(
                        product_code, eqp_id)]
                else:
                    tact_times[eqp_id] = self.tact_time_imputed_table[(
                        product_code, eqp_id)]
    ...
    def group_into_wip(self):
        ...
        for sheet_info_list_in_wip in wip_groups.values():
            ...
            tact_times = self._get_tact_times(
                route_id, op_id, product_code, on_ODF)
