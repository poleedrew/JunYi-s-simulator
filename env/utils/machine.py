...
class ODFMachine:
    ...
    def process_one_wip(self, wip_info, start_time, history):
        ...
        if setup_time != timedelta(hours=0):
            setup_info = {}
            setup_info["sheet_status"] = "DMQC"
            setup_info["selected_eqp_id"] = self.eqp_id
            setup_info["model_abbr"] = "DMQC"
            setup_info["booking"] = "DMQC"
            if self.current_time < start_time:
                setup_info['min_qtime'] = self.current_time
                setup_info['start_time'] = self.current_time
                setup_info['finish_time'] = self.current_time + setup_time
            else:
                setup_info['min_qtime'] = start_time
                setup_info['start_time'] = start_time
                setup_info['finish_time'] = start_time + setup_time
            setup_info["size"] = 0
            # history.append({
            #     "sheet_status": "DMQC",
            ...
            # })
            history.append(setup_info)
            self.setup_time_hist.append([self.current_time, self.current_time + setup_time])
            if setup_info['finish_time'] > start_time:
                start_time = setup_info['finish_time']
        
        wip_info['selected_eqp_id'] = self.eqp_id
        wip_info['start_time'] = start_time
        ...
        assert(self.not_overlap_in_booking(wip_info['start_time'], wip_info['finish_time']))
